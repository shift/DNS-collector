    ...
    def test_stdout_recv(self):
        ...
        async def run():
            ...
            args = ( 
                    "./dnscollector",
                    "-config",
                    "./tests/testsdata/config_stdout_powerdns.yml",
                    )
            transport_collector, protocol_collector =  await loop.subprocess_exec(
                lambda: ProcessProtocol(is_ready, is_clientresponse),
